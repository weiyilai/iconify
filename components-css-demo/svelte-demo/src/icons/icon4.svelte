...
/** @type {{mode?: 'auto' | 'light' | 'dark'; fill?: 'no-fill' | 'light-filled' | 'dark-filled' | 'filled'; focus?: boolean; static?: boolean; width?: string; height?: string;}} */
let {mode: modeProp, fill: fillProp, focus: focusProp, static: staticProp, width: widthProp, height: heightProp, ...props} = $props();

let states = $derived(({ 'mode': namedStateValue(modeProp, 'auto'), 'fill': namedStateValue(fillProp, 'no-fill'), 'focus': focusProp, 'static': staticProp }));
